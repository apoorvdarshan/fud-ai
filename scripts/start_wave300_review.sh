...
cd "$ROOT"
python3 "$ROOT/scripts/build_cursor_wave_300.py"
BATCH="cursor-wave-300"
...
if [[ ! -d "$CAND" ]] || [[ $(find "$CAND" -maxdepth 1 -name '*.png' | wc -l) -lt 2400 ]]; then
  echo "Missing combined candidate PNGs" >&2
  exit 1
fi
