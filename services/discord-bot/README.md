...
The same Worker wakes hourly and reads the Play production track. A new version code is posted once in **#announcements**, with the English What's New. The first run only records the versions already live, so it does not announce them again. Open testing is not announced. Secrets: `DISCORD_BOT_TOKEN` and `PLAY_SERVICE_ACCOUNT_JSON`.
