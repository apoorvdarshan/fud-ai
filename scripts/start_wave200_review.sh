...
if [[ ! -d "$CAND" ]] || [[ $(find "$CAND" -maxdepth 1 -name '*.png' | wc -l) -lt 1600 ]]; then
  echo "Missing candidate PNGs — run collect_cursor_cloud_candidates.py --batch $BATCH --all" >&2
  exit 1
fi
lsof -ti :8768 2>/dev/null | xargs kill -9 2>/dev/null || true
export WORKOUT_REVIEW_PORT=8768
